...
public class VideoStore {
    ...
    public static void menu(int cid, Query q, String commands) throws Exception {
		...
		try
		{
			System.out.println("You can rent " + q.helper_compute_remaining_rentals(cid) + " more movies");
		}
		catch(Exception e)
		{
			System.out.println("Error finding movies left to rent");
		}
	    }
                ...
                /* find the movie with the given movie id */
                if (st.hasMoreTokens()){
                    try{
                        int mid = Integer.parseInt(st.nextToken());
                        System.out.println(q.getMovieFromId(mid));
                    } catch (Exception nfe){
                        System.out.println("Error: Cant find movie name");
			nfe.printStackTrace();
                    }
                }
                else{
        ...
        if (cid >= 0)
	{
		...
		for(int i = 2; i < args.length; i++)
		{
        	...
        	menu(cid, q, con); /* menu(...) does the real work */
	}
        else
            System.out.println(-1); /* innocent mistake, or malicious attack ? */
        q.closeConnection();
    }
